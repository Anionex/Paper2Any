...
**配置说明**：
- `SAM3_GPUS`: 分配给 SAM3 的 GPU 列表
- `SAM3_START_PORT`: 第一个 SAM3 实例的端口
...
3. 启动 SAM3 集群（每个 GPU 一个实例）
4. 启动负载均衡器（MinerU LB: 8010, SAM3 LB: 8020）
5. 启动 OCR 服务（端口 8003）

**日志文件**：
- MinerU: `logs/mineru_gpu{gpu_id}.log`
- SAM3: `logs/sam_{gpu_id}.log`
...
如果你要在单张 GPU 上快速联调 `SAM3 + FastAPI(8009) + Frontend`，可直接执行：
...
默认行为：
- SAM3: `GPU=0`，端口 `8001`
- Backend: `uvicorn fastapi_app.main:app --host 0.0.0.0 --port 8009`
- Frontend: `npm run dev`（端口 `3000`）

常用覆盖参数示例：

```bash
SAM3_GPU=1 BACKEND_PORT=8009 FRONTEND_PORT=3000 bash script/start_local_sam3_dev.sh
```
